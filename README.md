...
`CLUSTER`            | `assemble_raw_reads` `resequence_assembly` | "LSF_PSP" | Sets the [`-D CLUSTER=`][smrtpipe] option for `smrtpipe.py`, which controls which job submission wrapper scripts are used. Set to `BASH` to disable job submissions by SMRT Pipe (all steps run local to the current node).
