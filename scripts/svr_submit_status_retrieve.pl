...
if ($result->{status} eq q(error)) {
    ...
    print qq($result->{job_id});
}
#my $res=$client->status_of_RAST_job({-job=>$result->{job_id}});
#my $n=0;
#my $completed=0;
#while($n<20){
#    $n++;
#    my $status_hash=$res->{$result->{job_id}};
#    my $status=$status_hash->{status};
#    my $err_msg="";
#    $err_msg="(error message: $status_hash->{error_msg})" if $status eq 'error';
#    my $job=$result->{job_id};
#    print "status for job $job: $status $err_msg\n";
#    if($status=~/complete/){
#	my $res=$client->retrieve_RAST_job({-job=>$result->{job_id}, -format=> 'genbank', -filehandle=>\*STDOUT});
