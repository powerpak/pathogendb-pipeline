...
$file1=~s/.fasta//;
$file1=~s/.fa//;
system("/sc/orga/work/attieo02/MUMmer3.23/nucmer --maxmatch --nosimplify $file $file -p $file1"."_itself; /sc/orga/work/attieo02/MUMmer3.23/show-coords $file1"."_itself.delta > $file1"."_itself.coords");
#system("/sc/orga/work/attieo02/MUMmer3.23/show-coords $file1"."_itself.delta > $file1"."_itself.coords");
$fr->init_file($file);
my $i=0;
...
#	print length(substr($circularized_dna, 0, $rotated_start{$contigs[$i]}))."\n";
	print FH1 substr($circularized_dna, $rotated_start{$contigs[$i]}).substr($circularized_dna, 0, $rotated_start{$contigs[$i]});
